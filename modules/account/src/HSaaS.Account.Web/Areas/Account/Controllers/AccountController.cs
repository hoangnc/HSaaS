...
using HSaaS.Account.Web.Areas.Account.Controllers.Models;
...
using UserLoginInfo = HSaaS.Account.Web.Areas.Account.Controllers.Models.UserLoginInfo;
using IdentityUser = Volo.Abp.Identity.IdentityUser;
using Volo.Abp;
using Volo.Abp.Account;

namespace HSaaS.Account.Web.Areas.Account.Controllers
{
    ...
    public class AccountController : AbpController
    {
        ...
        [HttpPost]
        [Route("login")]
        public virtual async Task<HSaaSLoginResult> Login(UserLoginInfo login)
        {
        ...
        public virtual Task<HSaaSLoginResult> CheckPasswordCompatible(UserLoginInfo login)
        {
        ...
        public virtual async Task<HSaaSLoginResult> CheckPassword(UserLoginInfo login)
        {
            ValidateLoginInfo(login);
            ...
            {
                return new HSaaSLoginResult(LoginResultType.InvalidUserNameOrPassword);
            }
        ...
        private static HSaaSLoginResult GetAbpLoginResult(SignInResult result)
        {
            if (result.IsLockedOut)
            {
                return new HSaaSLoginResult(LoginResultType.LockedOut);
            }

            if (result.RequiresTwoFactor)
            {
                return new HSaaSLoginResult(LoginResultType.RequiresTwoFactor);
            }

            if (result.IsNotAllowed)
            {
                return new HSaaSLoginResult(LoginResultType.NotAllowed);
            }
            ...
            return new HSaaSLoginResult(LoginResultType.Success);
        }
