...
(function () {
    ...
    const reviewAndReleaseDocumentApiPath = '/api/document-management/documents/review-and-release';
    const reviewDocumentApiPath = '/api/document-management/documents/review';
    const getCompaniesApiPath = '/api/masterdata/companies';
    ...
    const _documentReplaceForLookupModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'DocumentManagement/Documents/DocumentLookup',
        modalClass: 'DocumentLookup'
    });
    ...
    function getReplaceForDocuments() {
        const documents = grdReplaceForDocuments.getData().filter(function (d) {
            if (d.name) {
                return true;
            }
            return false;
    ...
    /*Replace for documents*/
    $(function () {

        getDocuments();

        let selectedReplaceForDocumentRow;

        const initGrdReplaceForDocuments = function () {

            grdReplaceForDocuments = new Tabulator("#grdReplaceForDocuments", {
                height: "311px",
                layout: "fitColumns",
                selectable: 1,
                movableRows: true,
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        cellDblClick: function (e, cell) {
                            selectedReplaceForDocumentRow = cell.getRow();

                            _documentReplaceForLookupModal.open({
        ...
        document.getElementById("btnAddReplaceForDocuments").addEventListener("click", function () {
            grdReplaceForDocuments.addRow({ name: 'Please choose document..' });
        });

        //Delete row on "Delete Row" button click
        document.getElementById("btnRemoveReplaceForDocuments").addEventListener("click", function () {
            const selectedRows = grdReplaceForDocuments.getSelectedRows();
            if (selectedRows && selectedRows.length > 0)
                grdReplaceForDocuments.deleteRow(selectedRows[0]);
        });

        _documentReplaceForLookupModal.onResult(function (selectedDocuments) {
            if (selectedDocuments) {
                if (selectedDocuments && selectedDocuments instanceof Array) {
                    if (selectedDocuments.length > 0) {
                        selectedReplaceForDocumentRow.update(selectedDocuments[0]);
                    }
                }
            }
        });

        initGrdReplaceForDocuments();
    });
    ...
    $(function () {
        ...
        function review() {
            ...
            const replaceForDocuments = getReplaceForDocuments().map((document) => document.code);
            formData.set('ReplaceFor', replaceForDocuments.join(';'));

            abp.ajax({
                url: reviewDocumentApiPath,
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false
                ...
                abp.notify.success(l(
                    'MessageReviewDocumentSuccessed'),
                    "Review Document");
            });
        }

        function reviewAndRelease() {
            // Create an FormData object
            let formData = $("#formDocument").submit(function (e) {
                e.preventDefault();
                return;
