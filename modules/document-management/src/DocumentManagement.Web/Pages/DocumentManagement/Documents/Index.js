...
(function () {
    ...
    function setDocumentColor(document) {
        if (document) {
            if (moment() < moment(document.effectiveDate)) {
                document["Color"] = '#BBBBBB';
                return;
            }

            const daysLeft = moment(document.reviewDate).diff(moment(), 'days');

            if (daysLeft <= 30 && daysLeft >= 20) {
                document["Color"] = '#E4E4E4';
            } else if (daysLeft <= 10) {
                document["Color"] = '#CECECE';
            } else if (daysLeft <= 0) {
                document["Color"] = '#ED1D24';
            } else {
                document["Color"] = '#00A43B';
            }
        }
    }
            ...
            result = {
                data: documents.map(document => {
                    ...
                    if (document.relateToDocuments) {
                        ...
                        const relateToDocuments = document.relateToDocuments
                            .split(';')
                            .map(d => {
        ...
        function initGrdDocument() {
            ...
            grdDocument = new Tabulator("#grdDocument", {
                ...
                ajaxURLGenerator: function (url, config, params) {
                    ...
                    return url + '?' + urlParameters + `&filter=${txtSearch.val()}&advancedSearch=${$('#AdvancedSearch').is(':checked')}`;//+ "?params=" + encodeURI(JSON.stringify(params)); //encode parameters as a json object
                },
